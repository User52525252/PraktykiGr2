...
public class BMIActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        weight = (EditText) findViewById(R.id.editTextNumber);
    ...
    private void displayBMI(float bmi) {
        ...
        if (Float.compare(bmi, 15f) <= 0) {
            ...
            bmiLabel = getString(R.string.normalnawaga);
        } else if (Float.compare(bmi, 25f) > 0  &&  Float.compare(bmi, 30f) <= 0) {
            bmiLabel = getString(R.string.nadwaga);
        } else if (Float.compare(bmi, 30f) > 0  &&  Float.compare(bmi, 35f) <= 0) {
            bmiLabel = getString(R.string.otylosc1);
        } else if (Float.compare(bmi, 35f) > 0  &&  Float.compare(bmi, 40f) <= 0) {
            bmiLabel = getString(R.string.otylosc2);
        } else {
            bmiLabel = getString(R.string.otylosc3);
        }
